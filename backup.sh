...
DST="/Volumes/Backup HD/"
SRC="/"
EXCLUDE="$HOME/bin/backup/exclude_list"
INCLUDE="$HOME/bin/backup/include_list"
...
if [ ! -r "$SRC" ]; then
    logger -s -t $PROG "Source $SRC not readable - Cannot start the sync process"
    exit;
fi

if [ ! -w "$DST" ]; then
    logger -s -t $PROG "Destination $DST not writeable - Cannot start the sync process"
    exit;
fi
